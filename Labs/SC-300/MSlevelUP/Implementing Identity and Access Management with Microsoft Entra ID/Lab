...
**Note** - For Conditional Access Policies, you can turn off Security Defaults, the key points to remember are from the training. Additional information on Security defaults can be found at this link: [https://docs.microsoft.com/en-us/azure/active-directory/fundamentals/concept-fundamentals-security-defaults](https://docs.microsoft.com/en-us/azure/active-directory/fundamentals/concept-fundamentals-security-defaults)
...
1.  Browse to https://entra.microsoft.com and sign in using a Global administrator account for the directory.
...
5.  On the **Overview (Preview)**, click **\+ Create new policy**.
    ...
    ![Screen image displaying the Conditional Access page with New policy highlighted](https://raw.githubusercontent.com/MicrosoftLearning/SC-300-Identity-and-Access-Administrator/refs/heads/CloudSlice-and-M36tenant/Instructions/Labs/media/lp2-mod1-conditional-access-new-policy.png)
...
11.  Verify **resources** is selected and then select **Select resources**, then select **None** in the select section.
...
15.  Under **Enable policy**, select **On**, and then select **Create**.
    ...
    ![Screen image displaying a new conditional access policy with policy settings highlighted](https://raw.githubusercontent.com/MicrosoftLearning/SC-300-Identity-and-Access-Administrator/refs/heads/CloudSlice-and-M36tenant/Instructions/Labs/media/lp2-mod3-create-conditional-access-policy.png)
...
1.  Open a new 'InPrivate' browser tab and then browse to https://sway.office.com.
...
2.  Verify you are prevented from accessing Microsoft Sway.
    ...
    ![Screen image displaying a the blocked resource access due to an enabled conditional access policy](https://raw.githubusercontent.com/MicrosoftLearning/SC-300-Identity-and-Access-Administrator/refs/heads/CloudSlice-and-M36tenant/Instructions/Labs/media/lp2-mod3-test-conditional-access-policy.png)
...
2.  On the menu, under **Identity**, select **Protection**.
...
6.  Under **User or Workload identity**, select **No user or service principal selected**.
    
7.  Choose **DebraB** as the user.
    
8.  Under **Cloud apps, actions, or authentication context**, select **Sway**.
    
9.  Select **What if**. You will be provided with a report at the bottom of the tile for **Policies that will apply** and **Policies that will not apply**.
...
5.  On the top menu, select **Policies**, then select **\+ New policy** from the drop-down select **Create a new policy**.
...
10.  Select **Target resources**.
    
11.  Verify **Resources** is selected and then select **Select resources**.
...
17.  Under **Enable policy**, select **Report-only**, and then select **Create**.
    ...
    ![Screen image displaying a new conditional access policy with policy settings highlighted](https://raw.githubusercontent.com/MicrosoftLearning/SC-300-Identity-and-Access-Administrator/refs/heads/CloudSlice-and-M36tenant/Instructions/Labs/media/lp2-mod3-create-session-conditional-access-policy.png)
